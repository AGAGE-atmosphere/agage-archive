...
- Combined-file comments now de-duplicate identical instrument comments (#175). The combined `comment` concatenated every contributing instrument's comment; instruments sharing an identical comment are now listed once, preserving `data_combination` order.
