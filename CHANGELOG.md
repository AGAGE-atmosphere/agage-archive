...
- **The combined baseline-flags file now reports its full instrument provenance** (#168). It inherited a single instrument's identity from the most recently operating instrument, so a baseline file spanning ALE+GAGE+GCMD+Medusa reported `instrument = instrument_type = "GCMS-Medusa"`. It now lists `instrument`, `instrument_1` … `instrument_n` and `instrument_type` (`"ALE/GAGE/GCMD/GCMS-Medusa"`) across all contributors, mirroring the combined mole fraction file. Instrument install dates are not available in the baseline inputs, so `instrument_date*` records each contributing instrument's first baseline timestamp within its `data_combination` window.
