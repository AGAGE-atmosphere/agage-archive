...
- **The archive is now more reproducible.** Two sources of potential run-to-run variation have been removed. Both could change the contents of a published archive between otherwise identical runs, so output may differ from previous versions — but it will now be the same every time.
...
- **Combined baseline flags could be misaligned with the combined mole fraction file they accompany** (#166). `combine_baseline` built the combined baseline independently of `combine_datasets`: it concatenated each instrument's baseline over the `data_combination` date windows, sorted by time and dropped duplicate timestamps blind — the code carried a comment noting the danger that this removed different points from the mole fraction dataset, because the instrument type was not available at that point. It also did not apply the combined-only data exclusions (`read_data_exclude(..., combined=True)`) that the mole fraction path applies. A timestamp measured by two overlapping instruments could therefore take its mole fraction from one and its baseline flag from the other. The combined mole fraction dataset is now passed in as the reference (`combine_baseline(..., reference_dataset=ds)`), and flags are looked up on `(time, instrument_type)` against it, so the baseline file matches its mole fraction file point for point; a combined data point with no corresponding flag now raises instead of being silently filled. Both products share one selection step (`_read_combined_instrument_datasets`), so the exclusions and date windows can no longer diverge.
...
- `read_data_exclude` and the resample aggregation (`define_agg_dict`) now skip variables that are not part of the output schema, instead of raising `KeyError`. Both run before `format_variables` in `read_nc`, so raw source variables such as `run_time` (present in some GCWerks netCDF files) are still on the dataset; tightening the exclusion's `time` skip to an exact match had left them reaching a `variable_defaults` lookup with no entry. Such variables are dropped by `format_variables` anyway, so skipping them changes no output.
...
- `read_ale_gage` no longer requires the network name to contain "agage" (#170), so the ALE/GAGE reader can be used by networks named otherwise.
...
- **Combined files now credit every contributing instrument's data owner** (#169). `data_owner` and `data_owner_email` on a combined mole fraction file were taken from the single instrument whose attributes were inherited, silently dropping the other owners. They are now the de-duplicated union of every contributing instrument's owners, ordered by the `data_combination` file, kept as the existing `", "`-separated string. Owners are paired with their email addresses positionally, matching the existing convention (`"Ray F. Weiss, Jens Muhle"` / `"rfweiss@ucsd.edu, jmuhle@ucsd.edu"`), so the two attributes stay aligned. No effect in the test fixture, where every instrument at a site shares one owner.
- **The combined baseline-flags file now reports its full instrument provenance** (#168). It inherited a single instrument's identity from the most recently operating instrument, so a baseline file spanning ALE+GAGE+GCMD+Medusa reported `instrument = instrument_type = "GCMS-Medusa"`. It now copies the complete instrument provenance — `instrument`, `instrument_1` … `instrument_n`, `instrument_type` and the matching `instrument_date*`/`instrument_comment*` — verbatim from the combined mole fraction file it accompanies, so the two files are consistent, including the type-prefixed detector identifiers described below.
- Combined-file comments now de-duplicate identical instrument comments (#175). The combined `comment` concatenated every contributing instrument's comment; instruments sharing an identical comment are now listed once.
- Combined-file comments are now ordered **most-recent instrument first**, matching the numbered `instrument_*` attributes (which are sorted newest-first). Previously the comment ran oldest-first while `instrument_*` ran newest-first, so the two lists disagreed in direction and a reader could not line them up. The same date sort is used for both.
- Numbered instrument attributes in combined files now **prefix each identifier with its instrument type** (#148), e.g. `instrument = "GCMS-Medusa - agilent_5975"`, so a reader can tell that an opaque identifier such as `agilent_5975` is a GCMS-Medusa. Identifiers that already begin with their type (`GCMD`, `GAGE_GCMD`, `ALE_GCMD`, `GCMS-ADS`) are left unchanged. Only combined files are affected; individual files, where the type is unambiguous from `instrument_type`, are unchanged.
- **Console output now uses the standard `logging` module instead of unconditional `print()`, and `run_all` is quiet by default.** Every `print()` call in the processing pipeline is now a `logging` call (routine progress at `INFO`, gated behind the existing `verbose` argument exactly as before; warnings and errors at `WARNING`/`ERROR`, always shown). `run_all` previously had no `verbose` parameter at all — despite a stale docstring line claiming it did — and hardcoded `verbose=True` for both the combined and individual processing passes, so every run printed several lines per species and made the progress bars impossible to read. It now has a real `verbose` parameter, defaulting to `False`. Two further issues this uncovered are also fixed:
  - `read_data_exclude`'s resample step (`convert.resample`) had no `verbose` parameter at all and printed unconditionally on every call, regardless of any setting — the single largest source of routine noise.
  - Consecutive identical messages (e.g. a per-site configuration gap that would otherwise warn once per species processed for that site) are collapsed to a single occurrence, so a real warning cannot be buried by its own repetition. The console handler resolves `sys.stderr` at write time rather than binding it when the logger is configured, so warnings and errors still appear when a notebook, IDE or debugger replaces the stream after import instead of being written to an abandoned one.
  Caught per-species/per-site processing errors are now also logged immediately (with site/species context) in addition to being written to `error_log_individual.txt`/`error_log_combined.txt` at the end of the run, so a failure is visible as it happens rather than only after the whole run finishes.
- **Zip archives now extract into a single top-level folder.** Both the `.nc` and CSV zips nest all their members under one self-named directory (`agage-public-archive-<version>/` and `agage-public-archive-csv-<version>/`, with the version taken from the network's `attributes.json`), rather than scattering their contents into whatever directory they are unzipped in. Previously only the CSV archive nested, and under its bare archive name; the `.nc` archive extracted at the top level. Directory-based outputs are unchanged, as a directory is already a named container. The pipeline still writes and reads members at archive-relative paths throughout — a zip is repackaged under its top-level folder as the final step of `run_all` — so combining, which reads files back out of the archive while it is being built, is unaffected.
